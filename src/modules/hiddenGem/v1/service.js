...
class HiddenGemService {
  ...
  async getHiddenGemsByTravelerId(travelerId) {
    try {
      const hiddenPlaces = await this.hiddenGemRepository.findByTravelerId(
        travelerId
      );
      const totalCount = await this.hiddenGemRepository.countByTravelerId(
        travelerId
      );
  ...
  async createNewHiddenGem({ hiddenGemData, imageFiles, userId }) {
    ...
    try {
      ...
      const traveler = await this.hiddenGemRepository.getTravelerByUserId(
        userId
      );
      ...
      const initialHiddenGem =
        await this.hiddenGemRepository.createInitialHiddenGem({
          ...hiddenGemData,
      ...
      console.log(
        "Initial hidden gem created with ID:",
        hiddenGemId,
        "Location ID:",
        locationId
      );
      ...
      if (imageFiles && imageFiles.length > 0) {
        ...
        primaryPictureId = await this.processPrimaryImage(
          hiddenGemId,
          imageFiles[0],
          userId
        );
        ...
        await this.hiddenGemRepository.updateHiddenGemWithPicture(
          hiddenGemId,
          primaryPictureId
        );
        ...
        if (imageFiles.length > 1) {
          await this.processAdditionalImages(
            hiddenGemId,
            imageFiles.slice(1),
            userId
          );
      ...
      const completeHiddenGem = await this.hiddenGemRepository.getHiddenGemById(
        hiddenGemId
      );
  ...
  async processPrimaryImage(hiddenGemId, imageFile, userId) {
    try {
      ...
      console.log(
        "Primary image processed successfully, media ID:",
        mediaRecord.id
      );
  ...
  async processAdditionalImages(hiddenGemId, imageFiles, userId) {
    try {
      ...
      console.warn(
        "Additional images failed to upload, but hidden gem creation continues"
      );
    }
  }
  ...
  async cleanupFailedCreation(hiddenGemId, locationId) {
    try {
      ...
      console.error(
        "Cleanup error during failed creation:",
  ...
  async updateHiddenGemStatus(hiddenGemId, statusData) {
    try {
      console.log("updateHiddenGemStatus service called with:", {
        hiddenGemId,
        statusData,
      });
      ...
      // Provide default rejection reason if not provided
      const finalRejectionReason =
        status === "rejected" && !rejectionReason
          ? "Rejected by moderator"
          : rejectionReason;

      console.log("Calling repository updateStatus with:", {
      ...
      console.log(
        "Repository updateStatus completed, returning:",
        updatedGem?.id
      );
      return updatedGem;
    } catch (error) {
